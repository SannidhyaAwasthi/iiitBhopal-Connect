    ...
    useEffect(() => {
        const fetchFavoritePosts = async () => {
            ...
            try {
                ...
                // Firestore 'in' query limitation (max 30 items per query as of recent updates)
                ...
                for (let i = 0; i < favoriteIds.length; i += chunkSize) {
                    ...
                    if (chunkIds.length === 0) continue; // Avoid empty queries

                    const postsRef = collection(db, 'posts');
                    // Note: Firestore allows querying by document ID using '__name__'
                 ...
                 const visiblePostIds = fetchedPosts.map(p => p.id); // Get IDs from fetched posts
                 ...
                 if (user && visiblePostIds.length > 0) {
                    try {
                       ...
                       // Continue without vote status
                    }
                 }

                // 4. Combine data and set state (all posts are favorites here)
                ...
                // Optionally sort favorites (e.g., by timestamp)
